...
/*
  Constructor
*/
ESPAdmin::ESPAdmin(AsyncWebServer *server, bool enable_stats)
{
  const char* ap_name = ("ESP_" + getID()).c_str();
  
      ESPConnect.autoConnect(ap_name);
  ...
  {
    Serial.println("Could not connect to webserver!");
  }
  _server = server;
  ...
  _server->on("/", HTTP_GET, [this](AsyncWebServerRequest *request) {
    ...
    AsyncWebServerResponse *response = request->beginResponse_P(200, "text/html", ADMIN_HTML, ADMIN_HTML_SIZE);
...
void ESPAdmin::setAuthentication(const char *user, const char *pass)
{
  username = user;
  password = pass;
  basic_auth = true;
...
// Remove Card
void ESPAdmin::remove(Card *card)
{
  for (int i = 0; i < cards.Size(); i++)
  {
    Card *p = cards[i];
...
// Remove Card
void ESPAdmin::remove(Chart *chart)
{
...
String ESPAdmin::generateUpdatesJSON(bool toAll)
{
  String cardsData;
  String chartsData;
...
String ESPAdmin::generateSettingsJSON()
{
  String output;
  ...
  output += "\"device_name\":\"ESP Lossnay\"";
...
// generates the layout JSON string to the frontend
String ESPAdmin::generateLayoutJSON(bool only_stats)
{
  String cardsData;
  String chartsData;
  String stats;
...
/*
  Generate Card JSON
*/
const String ESPAdmin::generateComponentJSON(Card *card, bool change_only)
{
  String data = "";
...
/*
  Generate Chart JSON
*/
const String ESPAdmin::generateComponentJSON(Chart *chart, bool change_only)
{
  String data = "";
...
void ESPAdmin::sendUpdates()
{
  _ws->textAll(generateUpdatesJSON(true));
}

void ESPAdmin::refreshLayout()
{
